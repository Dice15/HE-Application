...
namespace FHE.src.clients
{
    ...
    public partial class FourierTransformApplication : Form
    {
        ...
        {
            return ReEncryptValueAPI(encryptedComplex);
        }
        ...
        {
            var defaultConst = clientSeal.Encrypt(new Complex(1, 0));
            switch (n)
            {
                case 1:
                    {
                        var scaleConst2 = clientSeal.Power(defaultConst, 2);
                        for (int i = 0; i < d; i++)
                        {
                            x = clientSeal.Sum(
                                    clientSeal.Multiply(clientSeal.Power(x, 3), new Complex(-1.0 / 2.0, 0)),
                                    clientSeal.Multiply(clientSeal.Multiply(x, scaleConst2), new Complex(3.0 / 2.0, 0))
                                );
                            x = clientSeal.Encrypt(clientSeal.Decrypt(x));
                        }
                    }
                    break;
                case 2:
                    {
                        var (scaleConst2, scaleConst4) = (clientSeal.Power(defaultConst, 2), clientSeal.Power(defaultConst, 4));
                        for (int i = 0; i < d; i++)
                        {
                            x = clientSeal.Sum(
                                    clientSeal.Sum(
                                        clientSeal.Multiply(clientSeal.Power(x, 5), new Complex(3.0 / 8.0, 0)),
                                        clientSeal.Multiply(clientSeal.Multiply(clientSeal.Power(x, 3), scaleConst2), new Complex(-10.0 / 8.0, 0))
                                    ),
                                    clientSeal.Multiply(clientSeal.Multiply(x, scaleConst4), new Complex(15.0 / 8.0, 0))
                                );
                            x = clientSeal.Encrypt(clientSeal.Decrypt(x));
                        }
                    }
                ...
                case 3:
                    {
                        var (scaleConst2, scaleConst4, scaleConst6) = (clientSeal.Power(defaultConst, 2), clientSeal.Power(defaultConst, 4), clientSeal.Power(defaultConst, 6));
                        for (int i = 0; i < d; i++)
                        {
                            x = clientSeal.Sum(
                                    clientSeal.Sum(
                                        clientSeal.Multiply(clientSeal.Power(x, 7), new Complex(-5.0 / 16.0, 0)),
                                        clientSeal.Multiply(clientSeal.Multiply(clientSeal.Power(x, 5), scaleConst2), new Complex(21.0 / 16.0, 0))
                                    ),
                                    clientSeal.Sum(
                                        clientSeal.Multiply(clientSeal.Multiply(clientSeal.Power(x, 3), scaleConst4), new Complex(-35.0 / 16.0, 0)),
                                        clientSeal.Multiply(clientSeal.Multiply(x, scaleConst6), new Complex(35.0 / 16.0, 0))
                                    )
                                );
                            x = clientSeal.Encrypt(clientSeal.Decrypt(x));
                        }
        ...
        {
            return clientSeal.Multiply(x, Sgn(x, n, d));
        }

        private Ciphertext Max(Ciphertext a, Ciphertext b, int n = 1, int d = 14)
        {
            var fst = clientSeal.Sum(a, b);
            var snd = clientSeal.Sum(a, clientSeal.Negate(b));
            return clientSeal.Multiply(clientSeal.Sum(clientSeal.Multiply(fst, new Complex(1.0, 0)), Abs(snd)), new Complex(0.5, 0));
        }
        ...
        private Ciphertext Min(Ciphertext a, Ciphertext b, int n = 1, int d = 14)
        {
            var fst = clientSeal.Sum(a, b);
            var snd = clientSeal.Sum(a, clientSeal.Negate(b));
            return clientSeal.Multiply(clientSeal.Sum(clientSeal.Multiply(fst, new Complex(1.0, 0)), clientSeal.Negate(Abs(snd))), new Complex(0.5, 0));
        }
