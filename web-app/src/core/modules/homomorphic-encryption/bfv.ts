...
export class BFVSealBuilder {
    ...
    deserializePublicKey(serializedPublicKey: any): BFVSealBuilder {
    ...
    build(seal: SEALLibrary) {
        ...
        try {
            ...
            return new BFVSeal(seal, context, contextParms, encoder, keyGenerator, publicKey, secretKey, galoisKey, relinKeys, encryptor, decryptor, evaluator);
        }
        catch (e) {
            throw e;
        }
...
export class BFVSeal {
    ...
    private _contextParms: EncryptionParameters;
    ...
    constructor(
        seal: SEALLibrary,
        context: Context,
        contextParms: EncryptionParameters,
        ...
        this._contextParms = contextParms;
    ...
    sum(cipherText: CipherText, length: number = this._encoder.slotCount >> 1): CipherText {
        const logSlots = 1 << Math.ceil(Math.log2(length));
        let result = cipherText.clone();

        for (let step = 1; step < logSlots; step <<= 1) {
            const rotated = this._seal.CipherText();
            this._evaluator.rotateRows(result, step, this._galoisKey, rotated);
