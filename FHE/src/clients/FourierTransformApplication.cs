...
namespace FHE.src.clients
{
    ...
    public partial class FourierTransformApplication : Form
    {
        ...
        public Ciphertext ReEncryptComplexAPI(Ciphertext encryptedComplex)
        {
        ...
        /// </summary>
        private async Task Multiply(bool showCloudLog = false)
        {
            await Task.Run(() =>
            {
                // 결과 텍스트박스 초기화
                ...
                // 클라우드 생성
                FourierTransformService cloud = new FourierTransformService(
                    this,
                ...
                var transformedEncryptedVector1 = cloud.TransformAPI(encryptedVector1);
                var transformedEncryptedVector2 = cloud.TransformAPI(encryptedVector2);
                ...
                // 클라우드에 역변환 요청
                UIHelper.UpdateTextln(richTextBox_log, "Request transform encrypted vector (Client -> Cloud)", textColor: Color.Green);
                var encryptedVector3 = cloud.InverseTransformAPI(productTransformedEncryptedVector);
        ...
        private async void button_multiply_Click(object sender, EventArgs e)
        {
            ...
            {
                btn.Enabled = false;
                var dialogResult = MessageBox.Show("클라우드 로그를 확인하시겠습니까?", "로그 확인", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                await Multiply(showCloudLog: dialogResult == DialogResult.Yes);
                btn.Enabled = true;
            }
        }
        ...
        private void button_copy_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(textBox_result.Text))
            {
                ...
                MessageBox.Show("복사가 완료되었습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
