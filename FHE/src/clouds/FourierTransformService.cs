...
namespace FHE.src.clouds
{
    ...
    public partial class FourierTransformService : Form
    {
        ...
        public FourierTransformService(FourierTransformApplication client, SEALContext context, PublicKey publicKey, int maxMultiplyCount, Model model)
        {
            InitializeComponent();
            this.client = client;
            ...
            switch (model)
            {
                case Model.EncryptFFT:
                    this.fourierTransform = new EncryptFastFourierTransform(cloudSeal, ReEncryptComplex);
                    break;
                case Model.EncryptDFT:
                    this.fourierTransform = new EncryptDiscreteFourierTransform(cloudSeal, ReEncryptComplex);
                    break;
                default:
        ...
        public Ciphertext ReEncryptComplex(Ciphertext encryptedComplex)
        {
            UIHelper.UpdateTextln(richTextBox_log, "Request reencrypt complex (Client <- Cloud)", textColor: Color.Green);
            var result = client.ReEncryptComplexAPI(encryptedComplex);
            UIHelper.UpdateTextln(richTextBox_log, "Response reencrypted complex (Client -> Cloud)", textColor: Color.Green);
            UIHelper.UpdateTextln(richTextBox_log, "Return reencrypted complex (Cloud)", textColor: Color.Blue);
            return result;
        }
    }
}
